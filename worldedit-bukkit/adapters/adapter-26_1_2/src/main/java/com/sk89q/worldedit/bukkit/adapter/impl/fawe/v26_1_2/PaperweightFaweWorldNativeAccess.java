...
public class PaperweightFaweWorldNativeAccess implements WorldNativeAccess<LevelChunk,
    ...
    private static final AtomicInteger FALLBACK_TICK = new AtomicInteger();
    ...
    private int currentTick() {
        try {
            return getLevel().getServer().getTickCount();
        } catch (RuntimeException ignored) {
            return FALLBACK_TICK.incrementAndGet();
        }
    }
        ...
        if (sideEffectSet.shouldApply(SideEffect.EVENTS)) {
            ...
            if (craftWorld != null) {
                BlockPhysicsEvent event = new BlockPhysicsEvent(
                        ...
                        CraftBlockDataBridge.fromData(newState)
                );
